// src/rules/buttonEmojiOnly.ts
import * as vscode from "vscode";
import { A11yRule, makeDiagnostic } from "../utils/diagnostics";

/** Detects buttons whose only visible content is one or more emoji, which screen readers announce inconsistently. */
export const buttonEmojiOnlyRule: A11yRule = {
  id: "button-emoji-only",
  check(text, document) {
    /**
     * Matches <button> tags and captures their attributes and inner content.
     * NOTE: Regex is used for MVP. Replace with AST parsing for accurate HTML/JSX detection.
     */
    const buttonRegex = /<button\b([^>]*)>([\s\S]*?)<\/button>/gi;

    /** Removes nested HTML/JSX tags, leaving only text content. */
    const nestedTagRegex = /<[^>]+>/g;

    /**
     * Detects accessible name attributes that would make emoji content acceptable.
     * NOTE: Regex is used for MVP. Replace with AST parsing for accurate attribute parsing.
     */
    const accessibleNameRegex = /\b(?:aria-label|aria-labelledby|title)\s*=/i;

    /**
     * Matches strings composed entirely of emoji characters (and optional whitespace).
     * Covers the main Unicode emoji ranges.
     * NOTE: Regex is used for MVP; emoji detection via Unicode ranges may miss
     * some newer codepoints. Replace with a dedicated library or AST for accuracy.
     */
    const emojiOnlyRegex =
      /^[\s\u00a9\u00ae\u203c\u2049\u20e3\u2122\u2139\u2194-\u2199\u21a9-\u21aa\u231a-\u231b\u2328\u23cf\u23e9-\u23f3\u23f8-\u23fa\u24c2\u25aa-\u25ab\u25b6\u25c0\u25fb-\u25fe\u2600-\u2604\u260e\u2611\u2614-\u2615\u2618\u261d\u2620\u2622-\u2623\u2626\u262a\u262e-\u262f\u2638-\u263a\u2640\u2642\u2648-\u2653\u265f-\u2660\u2663\u2665-\u2666\u2668\u267b\u267e-\u267f\u2692-\u2697\u2699\u269b-\u269c\u26a0-\u26a1\u26a7\u26aa-\u26ab\u26b0-\u26b1\u26bd-\u26be\u26c4-\u26c5\u26ce-\u26cf\u26d1\u26d3-\u26d4\u26e9-\u26ea\u26f0-\u26f5\u26f7-\u26fa\u26fd\u2702\u2705\u2708-\u270d\u270f\u2712\u2714\u2716\u271d\u2721\u2728\u2733-\u2734\u2744\u2747\u274c\u274e\u2753-\u2755\u2757\u2763-\u2764\u2795-\u2797\u27a1\u27b0\u27bf\u2934-\u2935\u2b05-\u2b07\u2b1b-\u2b1c\u2b50\u2b55\u3030\u303d\u3297\u3299\ud83c-\ud83e\ufe0f\u200d]+$/u;

    return [...text.matchAll(buttonRegex)].flatMap((match) => {
      const attributes = match[1] ?? "";

      if (accessibleNameRegex.test(attributes)) {
        return [];
      }

      const visibleText = (match[2] ?? "").replace(nestedTagRegex, "").trim();

      if (visibleText.length === 0 || !emojiOnlyRegex.test(visibleText)) {
        return [];
      }

      return [
        makeDiagnostic(
          document,
          match.index,
          match[0].length,
          "Botão contém apenas emoji. Leitores de tela anunciam emoji de forma inconsistente. Adicione aria-label ou title com uma descrição textual.",
          vscode.DiagnosticSeverity.Warning,
        ),
      ];
    });
  },
};
