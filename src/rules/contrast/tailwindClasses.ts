...
function resolveTailwindTextColor(cls: string): RGB | null {
  // Arbitrary value: text-[#hex] or text-[rgb(...)]
  const arbitraryMatch = cls.match(/^text-\[([^\]]+)\]$/);
  if (arbitraryMatch) {
    const { parseColor } = require("../../utils/colorContrast");
    return parseColor(arbitraryMatch[1]);
  }

  const token = cls.replace(/^text-/, "");
  return TAILWIND_COLORS[token] ?? null;
}
...
function resolveTailwindBgColor(cls: string): RGB | null {
  const arbitraryMatch = cls.match(/^bg-\[([^\]]+)\]$/);
  if (arbitraryMatch) {
    const { parseColor } = require("../../utils/colorContrast");
    return parseColor(arbitraryMatch[1]);
  }
...
/**
 * Checks colour contrast for Tailwind CSS utility classes.
 *
 * Strategy:
 * 1. Find every JSX/HTML element opening tag.
 * 2. Extract its className / class attribute value.
 * 3. Look for text-* and bg-* colour classes.
 * 4. If both are present and resolvable, evaluate contrast.
 *
 * Limitations (MVP):
 * - Only analyses classes on the same element — inherited colours are not tracked.
 * - Dynamic class names (template literals, ternaries) are not resolved.
 * - Custom palette from tailwind.config.js is not read.
 * - Opacity modifiers (text-gray-500/75) are ignored (opacity not factored into contrast).
 *
 * NOTE: Replace with a Tailwind config-aware resolver and AST for production accuracy.
 */
export function checkTailwindClasses(
  ...
  /**
   * Matches className="..." or class="..." attribute values on any opening tag.
   * Also handles className={`...`} and className={'...'}.
   */
  const classAttrRegex =
    /\bclass(?:Name)?\s*=\s*(?:"([^"]*?)"|'([^']*?)'|\{`([^`]*?)`\}|\{['"]([^'"]*?)['"]\})/gi;

  /**
   * Finds the opening tag that contains a given attribute match,
   * so we can point the diagnostic at the element, not just the class string.
   */
  const openingTagRegex = /<[a-zA-Z][a-zA-Z0-9.-]*\b[^>]*>/g;

  // Pre-compute all opening tag spans for position lookup
  const tagSpans: Array<{ index: number; length: number }> = [];
  for (const m of text.matchAll(openingTagRegex)) {
    tagSpans.push({ index: m.index, length: m[0].length });
  }
  ...
  for (const match of text.matchAll(classAttrRegex)) {
    ...
    // Skip dynamic class strings we cannot resolve
    if (/\$\{|\?|&&|\|\|/.test(classValue)) {
      continue;
    }
    ...
    // Collect all text-* and bg-* colour tokens present on this element
    const textClasses = classes.filter(
      (c) =>
        /^(?:dark:)?text-(?!left|right|center|justify|start|end|ellipsis|clip|wrap|nowrap|balance|pretty|xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl|7xl|8xl|9xl|opacity|decoration|underline|overline|through|none|auto|inherit|current|transparent|black|white|[a-z]+-\d+|\[)/.test(
          c,
        ) || /^(?:dark:)?text-(?:white|black|transparent|\[[^\]]+\])/.test(c),
    );

    const bgClasses = classes.filter(
      (c) =>
        /^(?:dark:)?bg-(?!fixed|local|scroll|clip|origin|repeat|no-repeat|auto|cover|contain|none|gradient|opacity|\[url)/.test(
          c,
        ) &&
        /^(?:dark:)?bg-(?:white|black|transparent|[a-z]+-\d+|\[[^\]]+\])/.test(
          c,
        ),
    );
    ...
    // Try each text+bg combination (usually just one of each)
    for (const textCls of textClasses) {
      for (const bgCls of bgClasses) {
        const cleanText = textCls.replace(/^dark:/, "");
        const cleanBg = bgCls.replace(/^dark:/, "");
        ...
        // Find the last opening tag that starts before this class attribute.
        // Uses reduceRight instead of findLast to stay compatible with ES2020 target.
        const tagSpan = tagSpans.reduceRight(
          (
            found: { index: number; length: number } | undefined,
            span: { index: number; length: number },
          ) =>
            found === undefined && span.index <= match.index ? span : found,
          undefined,
        );

        const diagIndex = tagSpan?.index ?? match.index;
        const diagLength = tagSpan?.length ?? match[0].length;
        ...
        diagnostics.push(
          makeDiagnostic(
            ...
            formatContrastMessage(
              result,
              `classes Tailwind (${cleanText} / ${cleanBg})`,
            ),
            result.passesAA
